...
fn encode_arrow_schema(schema: &Schema) -> String {
    ...
    let mut dictionary_tracker = writer::DictionaryTracker::new_with_preserve_dict_id(
        true,
        options.preserve_dict_id(),
    );
    let data_gen = writer::IpcDataGenerator::default();
    let mut serialized_schema = data_gen.schema_to_bytes(schema, &mut dictionary_tracker, &options);
