...
pub fn schema_to_fb<'a>(schema: &'a Schema, dictionary_tracker: &'a mut DictionaryTracker) -> FlatBufferBuilder<'a> {
    let mut fbb = FlatBufferBuilder::new();

    let root = schema_to_fb_offset(&mut fbb, dictionary_tracker, schema);
...
pub fn schema_to_fb_offset<'a>(
    ...
    let fields = schema
        .fields()
        .iter()
        .map(|field| build_field(fbb, dictionary_tracker, field))
        .collect::<Vec<_>>();
...
pub(crate) fn build_field<'a>(
    ...
    let fb_dictionary = if let Dictionary(index_type, _) = field.data_type() {
        Some(get_fb_dictionary(
            index_type,
            dictionary_tracker.set_dict_id(field),
            field
                .dict_is_ordered()
...
mod tests {
    ...
    fn convert_schema_round_trip() {
        ...
        let fb = schema_to_fb(&schema, &mut dictionary_tracker);
    ...
    fn schema_from_bytes() {
        ...
        let bytes = data_gen
            .schema_to_bytes(&arrow_schema, &mut dictionary_tracker, &crate::writer::IpcWriteOptions::default())
            .ipc_message;
