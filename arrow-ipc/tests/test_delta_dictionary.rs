...
#[test]
fn test_write_with_delta_option() -> Result<(), ArrowError> {
    ...
    // For now, just verify it writes without error
    // Full delta functionality would be tested once implemented
...
#[test]
fn test_delta_dictionary_round_trip() -> Result<(), ArrowError> {
    // Test the full delta dictionary functionality with multiple batches

    // Create first batch with initial dictionary
    ...
    builder1.append_value("hello"); // duplicate
    ...
    // Create second batch with extended dictionary (delta case)
    ...
    builder2.append_value("foo"); // new value
    builder2.append_value("bar"); // new value
    builder2.append_value("hello"); // duplicate
    ...
    // The second batch should have all dictionary values available
    ...
    // Should have all 4 unique values: "hello", "world", "foo", "bar"
    assert_eq!(dict_values.len(), 4);
    assert_eq!(dict_values.value(0), "hello");
    assert_eq!(dict_values.value(1), "world");
    assert_eq!(dict_values.value(2), "foo");
    assert_eq!(dict_values.value(3), "bar");
...
#[test]
fn test_identical_dictionary_delta() -> Result<(), ArrowError> {
    // Test when dictionaries are identical (should not send delta)

    // First batch
    let mut builder1 = StringDictionaryBuilder::<arrow_array::types::Int32Type>::new();
    builder1.append_value("same");
    builder1.append_value("values");
    let array1 = builder1.finish();

    // Second batch with identical dictionary
    let mut builder2 = StringDictionaryBuilder::<arrow_array::types::Int32Type>::new();
    builder2.append_value("same");
    builder2.append_value("values");
    let array2 = builder2.finish();
    ...
    // Both batches should have identical dictionaries
    for batch in &read_batches {
        ...
        assert_eq!(dict_values.len(), 2);
        assert_eq!(dict_values.value(0), "same");
        assert_eq!(dict_values.value(1), "values");
    }
...
fn test_large_dictionary_delta_performance() -> Result<(), ArrowError> {
    ...
    assert!(buffer_size < buffer_no_delta_size, 
...
#[test]
fn test_dictionary_replacement_same_length() -> Result<(), ArrowError> {
    // Test the case where dictionary values change but length stays the same
    // This should send the full new dictionary, not attempt delta
    
    // First batch with values [0, 1, 3, 4, 5]
    let mut builder1 = PrimitiveDictionaryBuilder::<
        arrow_array::types::Int32Type,
        arrow_array::types::Int32Type,
    >::new();
    builder1.append_value(0);
    builder1.append_value(1);
    builder1.append_value(3);
    builder1.append_value(4);
    builder1.append_value(5);
    let dict1 = builder1.finish();
    
    // Second batch with values [0, 2, 3, 4, 5] - same length, different values
    let mut builder2 = PrimitiveDictionaryBuilder::<
        arrow_array::types::Int32Type,
        arrow_array::types::Int32Type,
    >::new();
    builder2.append_value(0);
    builder2.append_value(2); // Changed from 1 to 2
    builder2.append_value(3);
    builder2.append_value(4);
    builder2.append_value(5);
    let dict2 = builder2.finish();
    ...
    // Verify the second batch has the correct dictionary values
    let read_batch2 = &read_batches[1];
    let dict_array = read_batch2
        .column(0)
        .as_any()
    ...
    // Should have the new values [0, 2, 3, 4, 5]
    assert_eq!(dict_values.len(), 5);
    assert_eq!(dict_values.value(0), 0);
    assert_eq!(dict_values.value(1), 2); // Changed value
    assert_eq!(dict_values.value(2), 3);
    assert_eq!(dict_values.value(3), 4);
    assert_eq!(dict_values.value(4), 5);
...
fn test_dictionary_replacement_resend_mode() -> Result<(), ArrowError> {
    // Test that in Resend mode, changed dictionaries are sent as full replacements
